...
export function actorFromRequest(request: Request): Actor | null {
  ...
  if (email) {
    ...
    if (encodedName && encoding === "percent-encoded-utf-8") {
      try { fullName = decodeURIComponent(encodedName).trim().slice(0, 160) || email; } catch { fullName = email; }
    }
    return { email, fullName, isDemo: false };
  }

  if (process.env.NODE_ENV !== "production") {
    return { email: "demo@pawly.vn", fullName: "Minh Anh", isDemo: true };
  }
  return null;
}
...
export function validateMutationRequest(request: Request): string | null {
  ...
  if (!contentType.toLowerCase().startsWith("application/json")) return "Content-Type phải là application/json";
...
export async function ensureDatabase(db: D1Database) {
  ...
  await db.batch([
    ...
    db.prepare(`INSERT OR IGNORE INTO partner_users (email, partner_id, role) VALUES (?,?,?)`).bind("partner@pawly.vn", "vet-happy-paws", "owner"),
    db.prepare(`INSERT OR IGNORE INTO partner_users (email, partner_id, role) VALUES (?,?,?)`).bind("demo@pawly.vn", "vet-happy-paws", "owner"),
    db.prepare(`INSERT OR IGNORE INTO partner_users (email, partner_id, role) VALUES (?,?,?)`).bind("demo@pawly.vn", "shop-paw-mart", "manager"),
    db.prepare(`INSERT OR IGNORE INTO users (email, full_name, created_at) VALUES (?,?,?)`).bind("demo@pawly.vn", "Minh Anh", now),
    db.prepare(`INSERT OR IGNORE INTO pets (id,owner_email,name,species,breed,sex,date_of_birth,weight_kg,blood_type,microchip,allergies,notes,avatar,qr_token,created_at,updated_at) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)`).bind("pet-miso", "demo@pawly.vn", "Miso", "Chó", "Corgi Pembroke", "Đực", "2022-05-18", 11.8, "DEA 1.1-", "900215004812339", "Dị ứng nhẹ với thịt bò", "Năng động, thân thiện", "🐶", "pawly-miso-9fc12a", now, now),
    ...
    db.prepare(`INSERT OR IGNORE INTO appointments (id,owner_email,pet_id,partner_id,scheduled_at,reason,note,status,created_at,updated_at) VALUES (?,?,?,?,?,?,?,?,?,?)`).bind("appt-miso-001", "demo@pawly.vn", "pet-miso", "vet-happy-paws", "2026-07-17T09:30:00+07:00", "Khám da liễu", "Miso gãi tai 3 ngày", "confirmed", now, now),
    db.prepare(`INSERT OR IGNORE INTO appointments (id,owner_email,pet_id,partner_id,scheduled_at,reason,note,status,created_at,updated_at) VALUES (?,?,?,?,?,?,?,?,?,?)`).bind("appt-miso-002", "demo@pawly.vn", "pet-miso", "vet-happy-paws", "2026-07-18T15:00:00+07:00", "Tiêm phòng", "Nhắc mũi 7 bệnh", "pending", now, now),
